...
- A Gmail MCP server is connected (list_labels, delete_label, update_label, label/unlabel thread/message, search_threads, drafts, etc.). It exposes **no filter APIs** — filters can only be managed through the Gmail settings UI.
  - `search_threads` label queries take the label's **display name**, quoted if it contains spaces or punctuation (e.g. `label:"Read Later/Local"`) — not the label ID. The `label:Label_NN` ID form from `list_labels` **silently returns zero results** instead of erroring, so an empty result from it is never proof that a label is empty (verified 2026-09: an ID query on a >11,000-message label returned nothing; the name query returned its threads).
  - `delete_label` is the clean way to remove a label; confirm it is empty first with a name-based query (`search_threads` with `label:"<display name>" in:anywhere`) — an ID-based check will report empty regardless. The Gmail sidebar may keep rendering a deleted label until the page reloads.
