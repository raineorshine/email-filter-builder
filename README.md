...
Limited to the specific use case of matching by subject, sender, and mailing list, and filing into folders/labels.
...
- ProtonMail spec: https://proton.me/support/sieve-advanced-custom-filters
  ...
  - A condition's `list` becomes a `header :contains "List-Id"` test, since sieve has no dedicated mailing-list test.
...
- A condition's `list` maps to Gmail's `list:` operator, which matches the `List-Id` header. Use it for mailing lists whose sending address varies.
